...
final class Auth extends UserManager {
		...
		if (!$this->isLoggedIn()) {
			...
			if (isset($_COOKIE[$this->rememberCookieName])) {
				...
				if (!empty($parts[0]) && !empty($parts[1])) {
					try {
						$rememberData = $this->db->selectRow('SELECT a.user, a.token, a.expires, b.email, b.username, b.status, b.roles_mask, b.force_logout FROM ' . $this->makeTableName('users_remembered') . ' AS a JOIN ' . $this->makeTableName('users') . ' AS b ON a.user = b.id WHERE a.selector = ?', [$parts[0]]);
					} catch (Error $e) {
				...
				if (!$valid) {
					...
					$this->setRememberCookie('', '', time() + 60 * 60 * 24 * 365.25);
				}
			}
		}
	}
	...
	protected function onLoginSuccessful($userId, $email, $username, $status, $roles, $forceLogout, $remembered) {
		...
		try {
			$this->db->update($this->makeTableNameComponents('users'), ['last_login' => time()], ['id' => $userId]);
		} catch (Error $e) {
			throw new DatabaseError($e->getMessage());
		}
	...
	private function resyncSessionIfNecessary() {
		...
		if ($this->isLoggedIn()) {
			...
			if (($_SESSION[self::SESSION_FIELD_LAST_RESYNC] + $this->sessionResyncInterval) <= time()) {
				// fetch the authoritative data from the database again
				try {
					$authoritativeData = $this->db->selectRow('SELECT email, username, status, roles_mask, force_logout FROM ' . $this->makeTableName('users') . ' WHERE id = ?', [$this->getUserId()]);
				} catch (Error $e) {
					throw new DatabaseError($e->getMessage());
				}
	...
	private function authenticateUserInternal($password, $email = null, $username = null, $rememberDuration = null, callable $onBeforeSuccess = null) {
		...
		if ($email !== null) {
			...
			$userData = $this->getUserDataByEmailAddress($email, $columnsToFetch);
		} elseif ($username !== null) {
			$username = trim($username);

			// attempt to look up the account information using the specified username
			$userData = $this->getUserDataByUsername($username, $columnsToFetch);
	...
	private function createRememberDirective($userId, $duration) {
		...
		try {
			$this->db->insert($this->makeTableNameComponents('users_remembered'), ['user' => $userId, 'selector' => $selector, 'token' => $tokenHashed, 'expires' => $expires]);
		} catch (Error $e) {
			throw new DatabaseError($e->getMessage());
		}
	...
	public function confirmEmailAndSignIn($selector, $token, $rememberDuration = null) {
		...
		if (!$this->isLoggedIn()) {
			if ($emailBeforeAndAfter[1] !== null) {
				...
				$userData = $this->getUserDataByEmailAddress($emailBeforeAndAfter[1], ['id', 'email', 'username', 'status', 'roles_mask', 'force_logout']);
	...
	public function confirmEmail($selector, $token) {
		...
		try {
			$confirmationData = $this->db->selectRow('SELECT a.id, a.user_id, a.email AS new_email, a.token, a.expires, b.email AS old_email FROM ' . $this->makeTableName('users_confirmations') . ' AS a JOIN ' . $this->makeTableName('users') . ' AS b ON b.id = a.user_id WHERE a.selector = ?', [$selector]);
		} catch (Error $e) {
			throw new DatabaseError($e->getMessage());
		}
		...
		if (!empty($confirmationData)) {
			if (password_verify($token, $confirmationData['token'])) {
				if ($confirmationData['expires'] >= time()) {
					...
					try {
						$this->db->update($this->makeTableNameComponents('users'), ['email' => $confirmationData['new_email'], 'verified' => 1], ['id' => $confirmationData['user_id']]);
					...
					try {
						$this->db->delete($this->makeTableNameComponents('users_confirmations'), ['id' => $confirmationData['id']]);
					} catch (Error $e) {
						throw new DatabaseError($e->getMessage());
					}
					...
					return [$confirmationData['old_email'], $confirmationData['new_email']];
				} else {
	...
	public function reconfirmPassword($password) {
		if ($this->isLoggedIn()) {
			...
			try {
				$expectedHash = $this->db->selectValue('SELECT password FROM ' . $this->makeTableName('users') . ' WHERE id = ?', [$this->getUserId()]);
			} catch (Error $e) {
				throw new DatabaseError($e->getMessage());
			}
	...
	public function logOutEverywhereElse() {
		...
		if (isset($previousRememberDirectiveExpiry)) {
			...
			$this->createRememberDirective($this->getUserId(), $previousRememberDirectiveExpiry - time());
		}
	}
	...
	private function getRememberDirectiveExpiry() {
		...
		if ($this->isLoggedIn()) {
			...
			if (isset($existingSelector)) {
				...
				$existingExpiry = $this->db->selectValue('SELECT expires FROM ' . $this->makeTableName('users_remembered') . ' WHERE selector = ? AND user = ?', [$existingSelector, $this->getUserId()]);
	...
	public function changeEmail($newEmail, callable $callback) {
		if ($this->isLoggedIn()) {
			...
			try {
				$existingUsersWithNewEmail = $this->db->selectValue('SELECT COUNT(*) FROM ' . $this->makeTableName('users') . ' WHERE email = ?', [$newEmail]);
			} catch (Error $e) {
				throw new DatabaseError($e->getMessage());
			}
			...
			try {
				$verified = $this->db->selectValue('SELECT verified FROM ' . $this->makeTableName('users') . ' WHERE id = ?', [$this->getUserId()]);
			} catch (Error $e) {
				throw new DatabaseError($e->getMessage());
			}
	...
	private function resendConfirmationForColumnValue($columnName, $columnValue, callable $callback) {
		try {
			$latestAttempt = $this->db->selectRow('SELECT user_id, email FROM ' . $this->makeTableName('users_confirmations') . ' WHERE ' . $columnName . ' = ? ORDER BY id DESC LIMIT 1 OFFSET 0', [$columnValue]);
		} catch (Error $e) {
			throw new DatabaseError($e->getMessage());
		}
	...
	public function forgotPassword($email, callable $callback, $requestExpiresAfter = null, $maxOpenRequests = null) {
		...
		$userData = $this->getUserDataByEmailAddress($email, ['id', 'verified', 'resettable']);
	...
	private function getOpenPasswordResetRequests($userId) {
		try {
			$requests = $this->db->selectValue('SELECT COUNT(*) FROM ' . $this->makeTableName('users_resets') . ' WHERE user = ? AND expires > ?', [$userId, time()]);
	...
	private function createPasswordResetRequest($userId, $expiresAfter, callable $callback) {
		...
		try {
			$this->db->insert($this->makeTableNameComponents('users_resets'), ['user' => $userId, 'selector' => $selector, 'token' => $tokenHashed, 'expires' => $expiresAt]);
		} catch (Error $e) {
			throw new DatabaseError($e->getMessage());
		}
	...
	public function resetPasswordAndSignIn($selector, $token, $newPassword, $rememberDuration = null) {
		...
		if (!$this->isLoggedIn()) {
			...
			$userData = $this->getUserDataByEmailAddress($idAndEmail['email'], ['username', 'status', 'roles_mask', 'force_logout']);
	...
	public function resetPassword($selector, $token, $newPassword) {
		...
		try {
			$resetData = $this->db->selectRow('SELECT a.id, a.user, a.token, a.expires, b.email, b.resettable FROM ' . $this->makeTableName('users_resets') . ' AS a JOIN ' . $this->makeTableName('users') . ' AS b ON b.id = a.user WHERE a.selector = ?', [$selector]);
		} catch (Error $e) {
			throw new DatabaseError($e->getMessage());
		}
			...
			if ((int)$resetData['resettable'] === 1) {
				if (password_verify($token, $resetData['token'])) {
					if ($resetData['expires'] >= time()) {
						...
						try {
							$this->db->delete($this->makeTableNameComponents('users_resets'), ['id' => $resetData['id']]);
						} catch (Error $e) {
							throw new DatabaseError($e->getMessage());
						}

						return ['id' => $resetData['user'], 'email' => $resetData['email']];
					} else {
						throw new TokenExpiredException();
					}
				} else {
	...
	public function setPasswordResetEnabled($enabled) {
		...
		if ($this->isLoggedIn()) {
			try {
				$this->db->update($this->makeTableNameComponents('users'), ['resettable' => $enabled ? 1 : 0], ['id' => $this->getUserId()]);
			} catch (Error $e) {
				throw new DatabaseError($e->getMessage());
			}
		} else {
	...
	public function isPasswordResetEnabled() {
		if ($this->isLoggedIn()) {
			try {
				$enabled = $this->db->selectValue('SELECT resettable FROM ' . $this->makeTableName('users') . ' WHERE id = ?', [$this->getUserId()]);
