...
final class Base64 {
	...
	public static function encodeUrlSafe($data) {
		...
		return strtr($encoded, self::LAST_THREE_STANDARD, self::LAST_THREE_URL_SAFE);
	}
	...
	public static function encodeUrlSafeWithoutPadding($data) {
		...
		$encoded = rtrim($encoded, substr(self::LAST_THREE_STANDARD, -1));

		return strtr($encoded, substr(self::LAST_THREE_STANDARD, 0, -1), substr(self::LAST_THREE_URL_SAFE, 0, -1));
	}
	...
	public static function decodeUrlSafe($data) {
		$data = strtr($data, self::LAST_THREE_URL_SAFE, self::LAST_THREE_STANDARD);
