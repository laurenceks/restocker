...
foreach ($lists as $row) {
    if ($previousRow["locationId"] === $row["locationId"] && $previousRow["id"] === $row["id"]) {
        ...
        $listsTypes = array(&$output["lists"][count($output["lists"]) - 1], &$output["listsByLocationId"][$row["locationId"]][count($output["listsByLocationId"][$row["locationId"]]) - 1], &$output["listsByLocationThenListId"][$row["locationId"]][$row["id"]]);
